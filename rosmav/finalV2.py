...
class CombinedNode(Node):
    ...
    def tag_callback(self, msg):
        ...
        if len(tags) > 0:
            ...
            # TODO: Write code to lock in and go to it
            self.publish_desired_heading(int(np.rad2deg(angle)))
            self.get_logger().info(f"Distance to tag: {int(np.rad2deg(angle))}")
    ...
    def get_tag_distance(self, tag):
        ...
        return np.sqrt(position[0]**2 + position[1]**2 + position[2]**2) / 3.5
